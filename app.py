...
APP_VERSION = "short-description-fix-v1"
...
def recover_fields(text: str):
    ...
    # FORCE exact short description, no regex dependency
    ...
    fields["Founder Names + LinkedIn Profiles"] = grab(r"(Ganesh R \(CEO\):.*?tri2820/)")
    fields["Product Progress"] = "Beta--Private"
    fields["Product Progress Notes"] = grab(r"(We have moved beyond R&D into a production-ready stack.*?hardware\.)")
    ...
    fields["Traction/Revenue Notes"] = grab(r"(In the last 60 days, we have transitioned.*?within 90 days\.)")
    fields["Business Model + Unit Economics Notes"] = grab(r"(USA \+ Southeast Asia have 450,000\+ manufacturing plants\..*?core segment\.)")
